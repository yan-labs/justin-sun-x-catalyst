...
"""
    ...
    def test_binds_target_tweet_instead_of_adjacent_quoted_tweet(self):
        ...
        profile = (
            f'<a href="/justinsuntron/status/{target_id}"></a>'
        ...
        posts = parse_profile_posts(profile, "justinsuntron", user_id, "Justin Sun")
    ...
    def test_prefers_full_note_tweet_text_over_truncated_details(self):
        ...
        full_text = "Full NoteTweet text with $TRX and $HTX beyond the excerpt."
        profile = (
            f'<a href="/justinsuntron/status/{status_id}"></a>'
    ...
    def test_skips_profile_tweet_when_author_reference_does_not_match(self):
        requested_user_id = "902839045356744704"
        status_id = "889"
        token = base64.b64encode(f"Tweet:{status_id}".encode()).decode()
        ...
        self.assertEqual(parse_profile_posts(profile, "justinsuntron", requested_user_id, "Justin Sun"), [])
